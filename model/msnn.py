...
class MSNN(nn.Module):
    def __init__(self, Layers, parameter=1.5):
        super(MSNN, self).__init__()

        self.layers = Layers
        self.net = self.network()
        self.lambda0 = nn.Parameter(torch.tensor(parameter))
...
class MSNN2(MSNN):
    def __init__(self, Layers, initial_lambda=0.0):
        super(MSNN2, self).__init__(Layers, initial_lambda)
    ...
    def get_loss2(self, x_col):  # governing equation of example 2 for inver problem
        xc = x_col.requires_grad_()
        Uc = self.forward(xc)
        Ux = torch.ones_like(Uc)
        Uxx = torch.ones_like(Uc)
